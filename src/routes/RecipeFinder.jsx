        ...
        <div className='lg:px-12 px-4'>
          ...
          <form action="" className='py-4' onSubmit={handleSubmit}>
            <input type="text"
             ...
             required 
             value={meal}
             className='w-full py-3 px-8 rounded-full bg-neutral-100  border-2 border-neutral-300
             focus:ring-4 focus:ring-neutral-800 outline-none'
         ...
         { meal ? 
          <div className='grid gap-16'>
            {meals.map((meal) => (
              ...
              <h2 className='flex flex-wrap items-center gap-2 text-4xl lg:4xl text-neutral-800 font-bold mb-8'>
                {meal.strMeal} 
                <span className='bg-neutral-200 font-light text-sm px-2 rounded-full'>{meal.strCategory}</span>{''} <span
                className='bg-neutral-200 font-light text-sm px-2 rounded-full' >{meal.strArea}</span>
              </h2>

              <img src={meal.strMealThumb} alt={meal.strMeal} />

              <h3 className='items-center  text-2xl lg:2xl text-neutral-800 font-bold my-8' >Ingredient</h3>
              ...
              <h3 className='items-center  text-2xl lg:2xl text-neutral-800 font-bold my-8 zack'>Instructions</h3>
              <p className=' zack'>
              ...
              <ul className='items-center gap-2 justify-start flex mt-8  pl-4'>
                ...
                {meal.strSource && 
                 <li >
                  ...
                  className='bg-neutral-200 font-light text-base px-6 py-2 rounded-full hover:bg-neutral-400 transition-all duration-150 hover:text-neutral-800'>Source </a></li>
                }
              </ul>
            </article>
            ))}
          </div> : <h2 className='text-3xl lg:text-4xl text-neutral-400 font-bold mt-12'>Sorry, we couldn't search for {meal} </h2> }
        </div>
      </div>
    </div>
  )
